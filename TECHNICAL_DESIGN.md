...
**Испытания (`src/levels/elite-challenges.ts`):** не копируют уровень — ссылаются на `sourceLevelId`; пороги строятся из `par`/`par2` (`buildGoals`), поэтому золото = оптимум решателя (тест проверяет `solve(level).optimal ≤ gold.maxMoves` для всех 30). Первый дивизион открывается с позиции 65 кампании; до финала остальные дивизионы, недельный чемпионат и Испытания деда закрыты.
