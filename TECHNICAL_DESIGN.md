...
tests/         unit: game, solver, validator, levels (все 12: валидны, проходимы, par == оптимум)
...
```
Ход = скольжение одной фигуры на любую дистанцию вдоль оси. Ящик (`dir:"any"`) скользит по обеим осям, каждый ход тратит `maxMoves`. Звезда собирается, если любая фигура «прометает» её клетку. Победа — целевая машина полностью покидает поле через ворота (путь до края свободен); выезд = 1 ход.

## Решатель и валидатор
- **Решатель** — BFS по состояниям. Ключ состояния: позиции всех фигур + остатки ходов ящиков + флаг звезды. Находит: `optimal` (минимум ходов), `optimalWithStar` (минимум с собранной звездой), первый ход кратчайшего пути (используется в игре как подсказка за rewarded). Поля 6×6 с ≤10 фигурами — тысячи состояний, миллисекунды.
- **Валидатор**: выход за границы, пересечения, ровно один целевой, соосность целевого с воротами, ворота в границах, неизвестные типы, звезда/стены на занятых клетках, лимиты ящиков; затем решатель подтверждает проходимость. Всё это гоняется в `levels.test.ts` для каждого уровня — «ручной уверенности» нет.
...
## Сохранения
`SaveData { v, stars: {levelId: 0..3}, sound, lastLevel }` — через `Platform.saveData`. Mock — localStorage (`parkovka.save.v1`). Яндекс — `player.setData` с fallback на localStorage (без авторизации/оффлайн). Слияние: максимум звёзд по уровню.
...
- `ysdk.adv.showFullscreenAdv({callbacks})` — между уровнями, не чаще раза в 3 уровня; на `onOpen` — mute + gameplayStop, на `onClose` — восстановление.
