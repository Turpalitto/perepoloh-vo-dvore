 ...
 * сам режим открывается задолго до финала. Лига появляется позже отдельным
 * тизером, поэтому на пороге Endless её ещё быть не должно.
 */
...
test.describe('доступ к Бесконечному двору', () => {
  ...
  test('Высшая лига ещё не появляется на раннем пороге Endless', async ({ page }) => {
    await seedCleared(page, UNLOCK_AT);
