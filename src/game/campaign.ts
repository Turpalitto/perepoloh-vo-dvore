 ...
 *
 * Глава 10 «Всё сразу» запланирована как отдельный блок после финального босса
 * главы 9: лёд, куры и held-кнопка должны встречаться в ней вместе и попарно,
 * а не по одной. Уровни ещё не дозгенерированы (найдено 8 из 12), поэтому в
 * таблице её пока нет — тест «таблица покрывает кампанию ровно один раз» не
 * даст добавить главу раньше уровней. Переводы `chapter.10` во всех языках уже
 * на месте; при добавлении блока достаточно вернуть двенадцатёрку в массив.
 *
 * Инварианты (`levels.test.ts`): сумма размеров равна числу уровней, и у каждой
 * главы есть перевод `chapter.N` во всех языках.
 */
const CHAPTER_SIZES: number[] = [12, 12, 12, 12, 12, 16, 12, 16, 12];
