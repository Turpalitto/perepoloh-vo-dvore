 ...
 */
export type Daytime = 'dawn' | 'day' | 'evening' | 'night';

const VALID: Daytime[] = ['dawn', 'day', 'evening', 'night'];

export function currentDaytime(date: Date = new Date()): Daytime {
...
export function applyDaytime(): void {
  const override = new URLSearchParams(location.search).get('daytime') as Daytime | null;
