...
/**
 * Ранги. Максимум: 30 испытаний × 50 за золото = 1500 (дивизион 6 с новыми
 * механиками движка добавлен позже пятью капстоун-испытаниями поверх
 * исходных 25 — пороги ниже считались под них и остались нетронутыми).
 *
 ...
 * игрок в кампании прошёл на три звезды, поэтому «все 30 серебром» = 750 очков
 * должно означать не золотой ранг, а серебряный: медали собраны, ни одного
 * оптимума не выбито.
 *
 * Отсюда шкала: 750 (всё серебро) остаётся ниже `gold`, а дальше ранг растёт
 * ровно по числу золотых медалей — каждая даёт +25 сверх серебра.
 * gold ⇒ ≥5 золотых, champion ⇒ ≥13, legend ⇒ ≥21 из 30.
 *
 * Верхние три порога пересчитаны при росте лиги с 25 испытаний до 30. Прежний
 * `gold` был 750 — ровно столько теперь даёт «всё серебром», то есть золотой
 * ранг выдавался бы за нулевое число выбитых оптимумов. Это не запас
 * прочности, а тихая инверсия смысла шкалы, поэтому пороги привязаны к числу
 * испытаний, а не оставлены как были.
 */
export const RANKS: RankDef[] = [
  ...
  { key: 'gold', points: 875 },
  { key: 'champion', points: 1075 },
  { key: 'legend', points: 1275 }
];
