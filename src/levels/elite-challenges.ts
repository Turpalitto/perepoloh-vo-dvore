 ...
 *
 * Модификаторов «none» осталось 5 из 30: испытание со знакомой геометрией и без
 * единого ограничения отличается от кампании только счётчиком ходов, и таких
 * должно быть меньшинство. Распределение: none 5, noUndo 10, noHints 7,
 * noUndoNoHints 8 (проверяется тестом).
 *
 * Дивизион 6 (id 26–30) добавлен отдельно от кураторского перебора 1–25:
 * капстоун-блок на held-кнопке (26), курах (28, 30, 29) и льду (27). Каждое —
 * ремикс уровня, который в первых 25 испытаниях ещё не использован
 ...
 * (16, 17, 19, 20, 20).
 *
 * Held-кнопка представлена одним испытанием, а не двумя: среди уровней
 * кампании с `gateSwitch` режим `held` поднимает оптимум ровно на двух (48:
 * 10→13 и 56: 11→16), и первый слишком легок для этого дивизиона. Ставить
 * held-испытание с `par 13` после `par 17` значило бы сломать возрастание
 * нагрузки ради формальной симметрии механик.
 *
 * Хрупких досок здесь больше нет, хотя блок задумывался как капстоун именно
 * новых механик движка. Два прежних испытания на досках (источники 82 и 85)
 * оказались пустыми: оптимум с доской, со льдом и с обычной СТЕНОЙ совпадал —
 * 17/17/17 и 18/18/18. Перебор 1228 постановок по кампании и генерируемым
 * раскладам не нашёл ни одной клетки, где разрушение доски поднимает оптимум:
 * доска играет, только если решение пересекает клетку дважды, а в кратчайших
 * решениях на 6×6 этого не бывает. Подробности и ужесточённая проверка — в
 * `scripts/verify-planks.ts`.
 *
 * Замены подобраны решателем на свободных источниках с сохранением лестницы и
 * раскладки модификаторов (оба прежних были `noHints`): 96 + ледяная клетка
 * (par 19) и 97 + курица (par 20). Лёд вместо доски — сознательная уступка:
 * тематически он слабее (в кампании уже есть), зато его вес доказан
 * `verify:ice`, а кур на источниках 96 и 99 решатель не нашёл вовсе.
 *
...
const CURATED: Array<{ source: number; modifier: EliteModifier; remix?: Omit<RemixSpec, 'source'> }> = [
  ...
  //
  // У каждого испытания есть своя `hint`, и это не косметика: правило игрок
  // видит впервые (в кампании этих механик нет), а модификаторы дивизиона
  // отбирают подсказку решателя. Без строки-объяснения новая механика читалась
  // бы как баг — «машина не встаёт на клетку», «ворота закрылись сами».
  {
    source: 56,
    modifier: 'noUndo',
    remix: {
      holdType: 'held',
      name: 'Затор у забора',
      hint: 'Кнопка держит ворота: съехал с неё — створки захлопнулись',
      par: 16,
      par2: 18
    }
  },
  {
    ...
    modifier: 'noUndoNoHints',
    remix: {
      chickens: [{ a: { x: 2, y: 1 }, b: { x: 0, y: 1 } }],
      name: 'Курятник в разгар уборки',
      hint: 'Курица перелетает после каждого хода — бледный курятник показывает куда',
      par: 17,
      par2: 19
    }
  },
  {
    source: 96,
    modifier: 'noHints',
    remix: { flip: 'x', ice: [{ x: 2, y: 3 }], name: 'Кузница во льду', par: 19, par2: 21 }
  },
  {
    source: 104,
    modifier: 'noUndoNoHints',
    remix: {
      chickens: [{ a: { x: 2, y: 0 }, b: { x: 1, y: 0 } }],
      name: 'Чай и курица',
      hint: 'Курица занимает клетку по очереди — считай ходы, чтобы поймать её на нужной',
      par: 20,
      par2: 22
    }
  },
  {
    ...
    remix: {
      flip: 'x',
      chickens: [{ a: { x: 1, y: 2 }, b: { x: 1, y: 3 } }],
      name: 'Курица на рубеже',
      hint: 'Курица перелетает после каждого хода — считай, где она окажется к твоему манёвру',
      par: 20,
      par2: 22
    }
  }
