 ...
 *
 * Дивизион 6 (id 26–30) добавлен отдельно от кураторского перебора 1–25: пять
 * капстоун-испытаний на новых механиках движка — held-кнопка (26), хрупкие
 * доски (27, 29), куры (28, 30). Каждое — ремикс уровня, который в первых 25
 * испытаниях ещё не использован (проверяется тестом на уникальность
 * источников). Порядок внутри блока и его первый элемент подобраны решателем
 * так же, как и для остальных дивизионов: не легче конца дивизиона 5 (par 14)
 * и не убывает внутри себя (16, 17, 17, 18, 20).
 *
...
const CURATED: Array<{ source: number; modifier: EliteModifier; remix?: Omit<RemixSpec, 'source'> }> = [
  ...
  },
  {
    ...
    remix: {
      planks: [{ x: 2, y: 0 }],
      name: 'Заросшая доска',
      hint: 'По доскам можно проехать, но не встать — и после проезда они проламываются',
      par: 17,
      par2: 19
    }
  },
  ...
  },
  {
    ...
    remix: {
      planks: [{ x: 3, y: 0 }],
      name: 'Тупик с провалом',
      hint: 'Доска выдержит один проезд: сломается — проход закрыт навсегда',
      par: 18,
      par2: 20
    }
  },
